...
<body class="flex font-[Content] font-bold w-full p-10 h-screen justify-center items-center" style="background-image: url({{ asset('images/show_bg.png') }}); background-size:cover; background-repeat:no-repeat; backdrop-filter: blur(2px);">
   ...
   <div class="lg:w-2/3 md:w-2/3 sm:w-2/3 w-full border-2 h-auto justify-center p-5 bg-green-600  hover:bg-red-600 duration-500">
    <div class="mb-2">
        ...
        @method('PUT')
         ...
         <div class="row">
            <div class="flex flex-wrap mb-5">
                    ...
                    <input class="w-full outline-none p-2 text-green-600 "  type="text" name="title" value="{{ $slide->title }}" class="form-control" placeholder="ចំណងជើង">
            </div>
                <div class="flex flex-wrap mb-5">
                    <strong class="text-white mb-2">មើលរូបភាព</strong>
                    <textarea class="w-full p-2 outline-none text-green-600" class="form-control" style="height:150px" name="link" placeholder="មើលរូបភាព">{{ $slide->title }}</textarea>
                </div>
                <div class="flex gap-5 mb-5 items-center text-white">
                    ...
                    <img class="border-2 p-2" src="/images/{{ $slide->image }}" width="300px">
                </div>
